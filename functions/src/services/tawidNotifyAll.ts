...
export const tawidNotifyAll = functions.onSchedule({
  ...
  // Configurable lead time (minutes before departure to notify).
  // Falls back to 30 if not set in company/details.
  const leadMinutes = companyDoc.data()?.notificationLeadMinutes ?? 30;
  ...
  for (const doc of tripsSnap.docs) {
    ...
    // Check if within the configured lead window
    const tripMinutes = h * 60 + m;
    let diff = tripMinutes - nowMinutes;
    if (diff < 0) diff += 1440;
    if (diff < 0 || diff > leadMinutes) continue;
    ...
    const randomMessages = [
      "Baydi, at kung anu-ano pa ang hinahasi, maiiwanan na kayo!",
      "Andu, malapit na — dalian ninyo at baka maiwanan pa!",
      "Mag-ready na, at maiiwanan na kayo ngani!",
      "Aba'y bilis-bilisan hane. Anu pa bang hinihintay mo?",
    ];

    // Pumili ng random message
    const randomMsg = randomMessages[Math.floor(Math.random() * randomMessages.length)];
    logger.info(`🎲 Picked message for ${trip.time}: "${randomMsg}"`);

    const body = `Ka-Tawid, ${leadMinutes} minuto na lang at aalis na ang ` +
      `${vesselName} (${vesselType}) ngayong ${trip.time}. ${randomMsg}`;
