...
export default function Schedule({ role, searchQuery = '', filters }: ScheduleProps) {
  ...
  const ScheduleItem = ({ item }: { item: typeof schedules[0] }) => {
    ...
    const tableRows = [
      { label: 'Type', value: item.boatType },
      { label: 'Status', value: item.status, color: item.status === 'Available' ? colors.success : colors.error },
      { label: 'Capacity', value: `${item.capacity} passengers` },
      { label: 'Departure', value: item.time }
    ];

    const available = item.status === 'Available' ? colors.success : colors.error;
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <View>
            ...
            <View style={{
              ...
              <Text style={{ fontSize: 15, color: available, }}>{item.status}</Text>
            </View>
            <View style={{
              flexDirection: 'row',
              alignItems: 'center',
              ...
              <Text style={{ fontSize: 15, color: colors.dark }}>
                {`${item.boatType} | ${item.capacity} pax`}
              </Text>
            </View>
          </View>
          <Text style={{ fontSize: 30, fontWeight: 'bold', color: colors.dark }}>₱{item.price}</Text>
        </View>
